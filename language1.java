 ...
 * 
 * Creates its own native language: QuatDnaLang, a quaternary Brainfuck variant optimized for DNA sequences.
...
public final class Float4096 {
     ...
     * - Supports all standard Turing operations: read, write, move left/right, state change, halt.
    ...
    public static final class DnaTuringMachine {
        ...
        public String run(int maxSteps) {
            ...
            if (tapeStr.isEmpty()) return String.valueOf(blankSymbol);
            return tapeStr;
        }
        ...
        // Example: Create a TM that increments a quaternary number (DNA sequence) by 1
        public static DnaTuringMachine createIncrementer(String initialDna) {
            Builder builder = new Builder().initialTape(initialDna).initialState("inc").haltState("halt").initialHeadPosition(initialDna.length() - 1); // Start at LSB
            // Move to rightmost (but since head at end, start increment)
            // Increment with carry
            for (char symbol : new char[] {'A', 'C', 'G', 'T'}) {
                int val = DNA_TO_QUAT.get(symbol);
                int newVal = quatSuccessor(val);
                char newSymbol = QUAT_TO_DNA.get(newVal);
                if (newVal == 0) {
                    // Carry
                    builder.addTransition("inc", symbol, "inc", newSymbol, 'L');
                } else {
                    // No carry, halt
                    builder.addTransition("inc", symbol, "halt", newSymbol, 'R');
                }
            }
            // If left of tape with carry, add new digit
            builder.addTransition("inc", 'A', "halt", 'C', 'R'); // Add 1 if carry at left end
    ...
    /**
     * Interpreter for QuatDnaLang, a Turing-complete language native to this library.
     * Programs are DNA strings, parsed in pairs for commands (4^2 = 16 possible, 8 used like Brainfuck).
     * Tape cells are quaternary (0-3), operations use library's quaternary gates.
     * Commands:
     * AA: + (successor)
     * AC: - (predecessor)
     * AG: > (right)
     * AT: < (left)
     * CA: . (output as DNA base)
     * CC: , (input as DNA base)
     * CG: [ (loop start if cell != 0)
     * CT: ] (loop end)
     * Optimized for DNA: Input/output are DNA strings, tape can be converted to/from BigInteger.
     * Elegant: Uses Deque for bracket matching, ArrayList for tape, safe bounds checking.
     */
    public static final class QuatDnaLangInterpreter {
        ...
        /**
         * Executes a QuatDnaLang program.
         * @param program DNA string (must be even length, valid bases).
         * @param input DNA input string.
         * @param maxSteps Max instructions to prevent infinite loops.
         * @return Output as DNA string.
         */
        public static String execute(String program, String input, int maxSteps) {
            ...
            for (int i = 0; i < program.length(); i += 2) {
                ...
                if (cmd == null) {
                    throw new IllegalArgumentException("Invalid command pair: " + pair);
                }
